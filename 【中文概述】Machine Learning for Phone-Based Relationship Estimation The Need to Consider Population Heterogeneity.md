...
* **预测建模**
  ...
  * 用SHAP进行特征重要性分析：
    ...
    在实验中，我们希望解释模型特征的相对重要性，但是由于使用auto-sklearn构建了各种机器学习模型的集合，因此无法直接访问特征权重。而**SHAP**是Python开发的一个"模型解释"包，可以解释任何机器学习模型的输出。其名称来源于**SH**apley **A**dditive ex**P**lanation，在合作博弈论的启发下SHAP构建一个加性的解释模型，所有的特征都视为“贡献者”。对于每个预测样本，模型都产生一个预测值，SHAP value就是该样本中每个特征所分配到的数值。传统的feature importance只告诉哪个特征重要，但我们并不清楚该特征是怎样影响预测结果的，而SHAP value最大的优势是SHAP能对于反映出每一个样本中的特征的影响力，而且还表现出影响的正负性。

    基本思想：计算一个特征加入到模型时的边际贡献，然后考虑到该特征在所有的特征序列的情况下不同的边际贡献，取均值，即某该特征的SHAPbaseline value。

    例如：A单独工作产生的价值为$v_{A}$，加入B后共同产生价值$v_{A,B}$,那么B的累加贡献是$v_{A,B}-v_{A}$.

#### 4、  Results: relationship prediction analysis

我们使用了四个特征集来进行我们的关系预测：
...
4）交流特征+人口统计学特征+语义位置特征

75%参与者的数据用来训练，25%用来做测试。为了在关系角色之间进行有意义的比较，我们需要建立不同的类别。

我们首先建立了六类：“住在一起的家人“、”分开住的家人“、“朋友”、“同事”、“Task”，但后来发现”分开住的家人“和“朋友”两类很容易混淆。因为我们的模型是希望去预测社会关系的结构，而不是去预测社会角色（比如说我们想要预测的是这个人跟不跟你一起住，而不是这个人是你的朋友还是你的家人，而那种不一起住的兄弟姐妹跟朋友之类的社会关系整体特征非常相似，应该是同一类的）。所以我们重新定义了类别，把“分开住的家人“和“朋友”归位一类：分开住的社会关系（“social separate”），把“住在一起的家人”和“非常重要的其他人”归为一类：在一起的家人（“family together”）

预测精确度以及两个权重值见table3，

4.2 feature importance

我们想了解哪些特征有助于模型预测。 在15个最重要的特征中，13个来自于交流模式，剩下两个对提高模型表现没什么很大的作用。交流量、规律性以及时态趋势交流特征模态（astemporal tendency communication feature modalities）对模型输出具有重大影响。而对于个人来说，最重要的特征是交流的总天数，这个特征可以把social separate和family together分开，因为这个特征既包含了交流的规律性又包含了交流量。通过SHAP分析， 我们也发现沟通的时间和数量是决定参与者与亲密关系的本质的关键因素。

我们还想观察某些特征对特定分类任务的贡献，比如对“工作”分类最有帮助的特征是星期一的文字和上午8点至下午12点之间的通话；关于“任务”类型，我们看到通信的总数以及未接来电比率对于该分类尤为重要，直觉上“任务”类型中联系人与参与者的交流量应比其他联系人类型低得多，并且“任务”联系人未接来电的比例应更高，这可能是由于缺乏重要性交流或参与人不想加入交流。在分类联系人类型时，某些语义位置特征很重要，我们可以使用SHAP量化它们的影响。比如在商店的通话，或者因为拜访的在差事地点进行的通话。 这两个功能对“family togethe”类的输出做出了最大的贡献，这是合理的结果，因为在与差旅者或购物者进行交流时，很可能会与参与者生活在一起的联系人进行交流。 我们看到语义位置特征可以针对特定的联系人类型将交流事件情景化。